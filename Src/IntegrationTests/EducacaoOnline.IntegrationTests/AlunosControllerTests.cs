...
namespace EducacaoOnline.IntegrationTests
{
    public class AlunosControllerTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        [Fact]
        public async Task ConsultaDeAlunoPorIdBemSucedida()
        {
            #region cadastro do usuário/aluno
            ...
            #region consulta do aluno por id

            var consultaResponse = await _httpClient.GetAsync($"api/alunos/{aluno.Id}");
            var alunoConsultado = await consultaResponse.Content.ReadFromJsonAsync<AlunoDto>();

            alunoConsultado.Should().NotBeNull();
            alunoConsultado.Id.Should().Be(aluno.Id);
            alunoConsultado.Nome.Should().Be(aluno.Nome);
            alunoConsultado.Email.Should().Be(aluno.Email);
            #endregion

        }

        [Fact]
        public async Task ConsultaDeAlunoPorEmailBemSucedida()
        {
            #region cadastro do usuário/aluno
            ...
            #region consulta do aluno por email

            var consultaResponse = await _httpClient.GetAsync($"api/alunos/email/{aluno.Email}");
            var alunoConsultado = await consultaResponse.Content.ReadFromJsonAsync<AlunoDto>();
        ...
        [Fact]
        public async Task ConsultaDeMatriculasDoAlunoBemSucedida()
        {
            #region cadastro do curso
            ...
            #region consulta das matriculas do aluno

            var consultaResponse = await _httpClient.GetAsync($"api/alunos/{aluno.Id}/matriculas");
            var matriculasConsultadas = await consultaResponse.Content.ReadFromJsonAsync<IEnumerable<MatriculaDto>>();

            matriculasConsultadas.Should().NotBeNull();
            matriculasConsultadas.Should().NotBeNullOrEmpty();
            #endregion
